...
class VoxelNetSicp(nn.Module):
    def __init__(self, args):
        super(VoxelNetSicp, self).__init__()
    ...
    def forward(self, data_dict):
        ...
        _, _, H0, W0 = vmfs.shape # feature map shape [2, 128, 208, 704]
        t_matrix = normalize_pairwise_tfm(data_dict['pairwise_t_matrix'], H0, W0, self.voxel_size[0])
        print("vmfs",vmfs.shape)
        # fusion
        if self.train:
            ego = vmfs[0,:,:,:].unsqueeze(0)
            fused_feature = self.fusion_net(vmfs, record_len, t_matrix)

            psm1, rm1 = self.rpn(ego)
            psm2, rm2 = self.rpn(fused_feature)
