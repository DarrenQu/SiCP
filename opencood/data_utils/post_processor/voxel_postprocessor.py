...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            # (H, W, anchor_num, 7)
            anchor_box = cav_content['anchor_box'] # [100,352,2,7]

            # classification probability
            prob = output_dict[cav_id]['psm']
            prob = F.sigmoid(prob.permute(0, 2, 3, 1)) #[1，100，352，2]
            prob = prob.reshape(1, -1) #[1， 70400]

            # regression map
            reg = output_dict[cav_id]['rm'] # [1,14,100,352]
            # convert regression map back to bounding box
            # (N, W*L*anchor_num, 7)
            batch_box3d = self.delta_to_boxes3d(reg, anchor_box) #[1,70400,7]
            mask = \
                torch.gt(prob, self.params['target_args']['score_threshold']) #[1,70400]
            mask = mask.view(1, -1) 
            mask_reg = mask.unsqueeze(2).repeat(1, 1, 7) #[1,70400,7]
